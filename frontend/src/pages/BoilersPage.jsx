...
import { Flame, AlertTriangle, Package, ChevronDown, ChevronRight, ThermometerSun } from "lucide-react";

function BoilerCard({ mfr }) {
  const [open, setOpen] = useState(false);
  const [tab, setTab] = useState("install");
  return (
    <Card className="border border-border rounded-sm" data-testid={`boiler-${mfr.id}`}>
...
export default function BoilersPage() {
  ...
  useEffect(() => {
    axios.get(`${API}/boilers`, { headers: { Authorization: `Bearer ${token}` } })
      ...
      <div className="flex flex-wrap gap-2" data-testid="boiler-section-nav">
        {[["manufacturers","Manufacturers"],["codes","Codes"],["piping","Piping Diagrams"]].map(([id, label]) => (
          <Button key={id} variant={section === id ? "default" : "outline"} size="sm"
            ...
            onClick={() => setSection(id)} data-testid={`boiler-nav-${id}`}>{label}</Button>
        ))}
      </div>

      {section === "manufacturers" && (
        <div className="space-y-3">
          {data.manufacturers.map((m) => <BoilerCard key={m.id} mfr={m} />)}
        </div>
      )}
