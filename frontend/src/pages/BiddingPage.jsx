...
export default function BiddingPage() {
  ...
  const [saveTarget, setSaveTarget] = useState(null); // material row being saved
  ...
  const saveToCommon = async (mat, shared = false) => {
    if (!mat.name.trim()) {
    ...
    try {
      await axios.post(
        ...
        { name: mat.name.trim(), unit_price: parseFloat(mat.unit_price) || 0, shared },
        { headers }
      );
      toast.success(
        shared
          ? `Shared "${mat.name}" with your team`
          : `Saved "${mat.name}" to favorites`
      );
      fetchCommonMaterials();
      setSaveTarget(null);
    } catch (err) {
  ...
  return (
    <div className="space-y-6" data-testid="bidding-page">
                              ...
                              >
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center gap-2 flex-wrap">
                                    ...
                                    {cm.team_id && (
                                      <Badge className="bg-[#003366] text-white text-[10px] px-1.5 py-0">
                                  ...
                                  <p className="text-xs text-muted-foreground">
                                    ...
                                    {cm.shared_by_name && (
                                      <span className="ml-2 italic">by {cm.shared_by_name}</span>
                                    )}
                                  </p>
                                </div>
                                <Button
      ...
      {/* Save Target Dialog: Personal vs Team */}
      <Dialog open={!!saveTarget} onOpenChange={(open) => !open && setSaveTarget(null)}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle className="font-heading uppercase">Save Material</DialogTitle>
          </DialogHeader>
          {saveTarget && (
            <div className="space-y-4">
              <div className="bg-muted p-3 rounded-sm">
                <p className="font-bold">{saveTarget.name}</p>
                <p className="text-xs text-muted-foreground">${(parseFloat(saveTarget.unit_price) || 0).toFixed(2)} / unit</p>
              </div>
              <p className="text-sm text-muted-foreground">Where do you want to save it?</p>
              <div className="grid gap-2">
                <Button
                  type="button"
                  variant="outline"
                  className="justify-start h-auto py-3"
                  onClick={() => saveToCommon(saveTarget, false)}
                  data-testid="save-personal-btn"
                >
                  <Star className="w-5 h-5 mr-3 text-amber-500" />
                  <div className="text-left">
                    <div className="font-bold">My Favorites</div>
                    <div className="text-xs text-muted-foreground">Only you can use this</div>
                  </div>
                </Button>
                <Button
                  type="button"
                  variant="outline"
                  className="justify-start h-auto py-3 border-[#003366]"
                  onClick={() => saveToCommon(saveTarget, true)}
                  data-testid="save-team-btn"
                >
                  <Users className="w-5 h-5 mr-3 text-[#003366]" />
                  <div className="text-left">
                    <div className="font-bold">Share with Team</div>
                    <div className="text-xs text-muted-foreground">All teammates can use this</div>
                  </div>
                </Button>
              </div>
            </div>
